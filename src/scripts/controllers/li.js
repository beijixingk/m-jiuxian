const liView = require('../views/li.art')
const liModel = require('../models/clearance')
const liListView = require('../views/li_list.art')
...
class Li{
    ...
    async renderer(list0){
        ...
        let liListHtml = liListView({
            list0
        })
        await  $('.main_li .banner ul').eq(this.count).html(liListHtml)
    ...
    async render(){
        ...
        let result=await liModel.get({
            state1:'dynamic',
            state2:'mob01ajax/150578',
            last:'pageNum=1'
        })
        let liHtml = liView({
            
        })
        $('.indexContainer').html(liHtml)

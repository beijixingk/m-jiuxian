const buyView = require('../views/buy.art')
const buyModel = require('../models/clearance')
const buyListView = require('../views/buy_list.art')
...
class Buy{
    ...
    async renderer(list0){
        ...
        let buyListHtml = buyListView({
            list0
        })
        await  $('.main_buy .banner ul').eq(this.count).html(buyListHtml)
    ...
    async render(){
        ...
        let result=await buyModel.get({
            state1:'dynamic',
            state2:'mob01ajax/151711',
            last:'pageNum=1'
        })
        let buyHtml = buyView({
            
        })
        $('.indexContainer').html(buyHtml)

const xiaoView = require('../views/xiao.art')
const xiaoModel = require('../models/clearance')
const xiaoListView = require('../views/xiao_list.art')
...
class Xiao{
    ...
    async renderer(list0){
        ...
        let xiaoListHtml = xiaoListView({
            list0
        })
        await  $('.main_xiao .banner ul').eq(this.count).html(xiaoListHtml)
    ...
    async render(){
        ...
        let result=await xiaoModel.get({
            state1:'dynamic',
            state2:'mob01ajax/150779',
            last:'pageNum=1'
        })
        let xiaoHtml = xiaoView({
            
        })
        $('.indexContainer').html(xiaoHtml)

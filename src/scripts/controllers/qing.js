const qingView = require('../views/qing.art')
const qingModel = require('../models/clearance')
const qingListView = require('../views/qing_list.art')
...
class Qing{
    ...
    async renderer(list0){
        ...
        let qingListHtml = qingListView({
            list0
        })
        await  $('.main_qing .banner ul').eq(this.count).html(qingListHtml)
    ...
    async render(){
        ...
        let result=await qingModel.get({
            state1:'dynamic',
            state2:'mob01ajax/150722',
            last:'pageNum=1'
        })
        let qingHtml = qingView({
            
        })
        $('.indexContainer').html(qingHtml)
